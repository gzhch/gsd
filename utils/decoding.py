...
def base_generate_wo_kv(model, tokenizer, input_ids, max_new_tokens=10, 
                  do_sample=False, top_k=0, top_p=0.85, temperature=0.2,
                  early_stop=False):
    ...
    with torch.no_grad():
        for step in range(max_new_tokens):
            ...
            output_ids = sample(logits, do_sample=do_sample, top_k=top_k, top_p=top_p, temperature=temperature)
            generate_ids[:, step] = output_ids
            current_input_ids = torch.cat([current_input_ids, output_ids], dim=1)
            #past_key_values = output['past_key_values']
...
def base_generate_custom(model, tokenizer, input_ids, max_new_tokens=10, 
                  do_sample=False, top_k=0, top_p=0.85, temperature=0.2,
                  early_stop=False):
    ...
    with torch.no_grad():
        for step in range(max_new_tokens):
            output = ana.custom_forward(model, input_ids=current_input_ids,
                    past_key_values=past_key_values,
                    use_cache=True)
            logits = output['logits'][:,-1:]
            output_ids = sample(logits, do_sample=do_sample, top_k=top_k, top_p=top_p, temperature=temperature)
...
def exact_self_speculative_generate_with_dropout(model, tokenizer, input_ids, max_new_tokens=10, early_stop=False,
                 max_step_draft=8, th_stop_draft=0.8, auto_th_stop_draft=False, auto_parameters=[1,0.5,0.9,1e-2,0.9],
                 do_sample=False, do_sample_draft=False, top_k=0, top_p=0.85, temperature=0.2, draft_config=None):
    step = 0
    step_draft = 0
    step_verify = 0
        ...
        while True:
            ...
            if step == 0:
                ...
                for step_draft in range(max_step_draft):
                    idx = 0
                    if draft_config is not None and draft_config['mode'] == 'layer_dropout_ladder_1':
                        draft_config['k'] = idx
                        idx = (idx + 1) % draft_config['step']
                    elif draft_config is not None and draft_config['mode'] == 'layer_dropout_ladder_2':
                        draft_config['k'] = step_draft // (max_step_draft // draft_config['step'])
                        
                    draft_output = ana.custom_forward(model, input_ids=draft_current_input_ids,
...
generate_fn_mapping = {
    'base_custom': base_generate_custom,
    'base': base_generate,
    'base_no_kv': base_generate_wo_kv,
    'exact_self_speculative_generate': exact_self_speculative_generate,
    'layer_dropout': exact_self_speculative_generate_with_dropout,
